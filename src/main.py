...
# from src.export_excel import export_articles_excel
...
def post_to_discord_safe(webhook_url: str, content: str) -> None:
    for msg in split_discord_message(content):
        r = requests.post(webhook_url, json={"content": msg}, timeout=30)
        if r.status_code >= 300:
...
def build_ai_input(items: List[Dict[str, str]]) -> str:
    return "\n".join(
        f"{i}. {it['title']} | 出典:{it['source']} | 日付:{it.get('date','')} | "
        f"カテゴリ:{it.get('category','')} | {it['url']}"
        for i, it in enumerate(items, start=1)
    )
...
"""
    ...
    return resp.json()["output"][0]["content"][0]["text"].strip()
...
def main():
    ...
    industry_keywords = [w for words in keywords.values() for w in words]

    # 1) fetch
    items = fetch_rss_items(sources, keywords)
    items = dedupe_by_url(items)
    items = [it for it in items if is_relevant_to_industry(it, industry_keywords)]
    ...
    RANGES = [
        ...
        (90, "daily_90.json"),
        (365, "daily_365.json"),
        (1095, "daily_1095.json"),
    ]

    for days, name in RANGES:
        payload = query_daily_counts(conn, days)
        write_daily_counts(payload, Path("docs/data") / name)

    write_dashboard_html()
    write_keywords(compute_keywords(conn, 30))
    ...
    for days, name in [
        ...
        export_articles_excel(
            conn,
            days,
            Path("docs/exports") / name
        )

    # 5) Discord
    top_items = pick_top(items, top_n)
    if use_ai:
        try:
            message = summarize_with_openai_jp(industry, top_items)
        except Exception:
            message = build_basic_message(industry, top_items, len(items))
    else:
        message = build_basic_message(industry, top_items, len(items))
