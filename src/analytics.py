...
# JSON出力（拡張：出力先指定）
# ----------------------------
def write_daily_counts(payload: Dict[str, Any], out_path: Path | None = None) -> None:
    """
    - out_path を指定 → その場所に出力
    - 未指定 → 既存の daily_counts.json（後方互換）
    """
    path = out_path if out_path is not None else OUT_DAILY
